...
class Keyboard {
    ...
    initListener() {
        ...
        function mouseRepeater(e) {
            ...
            if (!keyBtn || !keyBtn.classList.contains ('key')) return;
        ...
        function keyDown(e) {
            ...
            textarea.focus ();
            if ('ControlLeft' in activeKeys && 'AltLeft' in activeKeys) {
                if (lang == 'en') lang = 'ru';
                else lang = 'en';
        ...
        function getStrWithEnters() {
            ...
            function findNextBreakLength(strSource, nLeft, nRight) {
                ...
                else if (nRight == -1) nCurrent = nLeft * 2;
        ...
        function cursorDown() {
            ...
            posEnters = Array.from (valueWithBreaks.split ('').entries ())
                .filter ((i) => i[1] == '\n')
                .map ((i) => i[0]);
            ...
            textarea.focus ();
            for (let i = posEnters.length -1; i >= -1; i--) {
                if (i == -1) {
                    ...
                    posCursorNew = posCursor - posEnters[i] + posEnters[i+1];
                    lenNextMax = posEnters[i+2];
                    if (posCursorNew > lenNextMax) posCursorNew = lenNextMax;
                    [textarea.selectionStart, textarea.selectionEnd] = [posCursorNew, posCursorNew];
                    return;
        ...
        function cursorUp() {
            ...
            for (let i = 0; i <= posEnters.length; i++) {
                if (i == posEnters.length) {
                    if (i == 1) {
                        posCursorNew = posCursor - posEnters[i-1] + (posEnters[i-2] || 0) -1;
                    }
                    ...
                    if (i == 1) {
                        posCursorNew = posCursor - posEnters[i-1] + (posEnters[i-2] || 0) -i;
                    }
                    else posCursorNew = posCursor - posEnters[i-1] + (posEnters[i-2] || 0);
                    lenPrevMax = posEnters[i-1];
                    if (posCursorNew > lenPrevMax) posCursorNew = lenPrevMax;
